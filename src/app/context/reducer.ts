...
export default function reducer(state: AppState, action: AppAction): AppState {
  ...
  if (action.type === "removeCardFromDeck") {
    const deck = copy.decks.find(({ id }) => id === action.payload.deckId);
    if (deck) {
      const cardToDelete = deck.cards.findIndex(
  ...
  if (action.type === "moveCard") {
    ...
    if (deckFromMove && deckToMove) {
      const cardToMoveIndex = deckFromMove.cards.findIndex(
        ({ id }) => id === cardId
      );
      if (~cardToMoveIndex)
        deckToMove.cards.push(deckFromMove.cards[cardToMoveIndex]);
      if (!duplicate) deckFromMove.cards.splice(cardToMoveIndex, 1);
    }
  }

  if (action.type === "updateCard") {
    const deckToUpdate = copy.decks.find(
      ({ id }) => id === action.payload.deckId
    );
    if (deckToUpdate) {
      const cardToUpdate = deckToUpdate.cards.findIndex(
        ({ id }) => id === action.payload.card.id
      );
      deckToUpdate.cards[cardToUpdate] = action.payload.card;
    }
  }
